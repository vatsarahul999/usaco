package com.rahul.dm;
	...
	public class BalanceLog {
		public double balancePoint(Log log) {
			 // YOUR CODE HERE
			 double logLen = log.length();
			 double totalW  =  log.weightUpto(logLen);
			 double res = logLen/2;
			 double leftW  = log.weightUpto(res);
			 double rleg = logLen;
			 double rightW = totalW - leftW;
			 for(int i = 0; i <= logLen;i++){
				 double lP = log.weightUpto(i);
				 double rP = log.weightUpto(i+1);
				 if((totalW - lP )*(totalW-rP)<0){
					 leftW = i;
					 rightW = i+1;
					 while(leftW!=rightW){
					     System.out.println(res + " left W" + leftW+ " right W"+ rightW);
					     if(leftW> rightW){
					         res -=(res/2);
